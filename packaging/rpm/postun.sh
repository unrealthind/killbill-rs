#!/bin/sh
# killbill-rs .rpm %postun. See packaging/scripts/lib.sh for the contract.
# $1 == 0 : final erase (nothing to do — preun already stopped/disabled;
#           /etc/killbill is kept, same as a non-purge .deb remove).
# $1 >= 1 : upgrade — reload and try-restart against the new unit.
# shellcheck disable=SC2329  # inlined lib.sh helpers: this scriptlet calls only some
set -e

# >>> lib.sh  (inlined verbatim from packaging/scripts/lib.sh; CI checks sync)
